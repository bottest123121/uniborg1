...
async def await_event(client, event, pre):
    ...
    borg.remove_event_handler(handler)
    return message
...
@borg.on(events.NewMessage(outgoing=True, pattern="(?i)!spamids"))
async def handler(event):
    ...
    groupname = await await_event(borg, events.NewMessage(outgoing=True), message)
    ...
    number = await await_event(borg, events.NewMessage(outgoing=True), message)
    #this one only gets the join messages of the group and how many you like
    result = await borg(GetAdminLogRequest(
    ...
    starting = await await_event(borg, events.NewMessage(outgoing=True), message)
    message = event.respond("Wuhu. Last step: How many IDs should I show you?")
    ending = await await_event(borg, events.NewMessage(outgoing=True), message)
